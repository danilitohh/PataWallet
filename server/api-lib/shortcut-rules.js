import { assertBodySize, assertTrustedOrigin, allowMethod, json } from './http.js'
import { normalizeLabel, ruleSchema } from './shortcut-contract.js'
import { shortcutError } from './shortcut-server.js'
import { adminClient, authenticatedUser } from './supabase-server.js'

// Atiende altas y bajas de reglas de categorización compartiendo la función de Vercel con los mapeos.
export async function categoryRuleHandler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      const { error } = await admin.from('category_rules').delete().eq('id', String(req.body?.id || '')).eq('user_id', user.id)
      if (error) throw error
      return json(res, 200, { deleted: true })
    }
    const parsed = ruleSchema.safeParse(req.body)
    if (!parsed.success) return json(res, 400, { error: 'Regla inválida.' })
    ...
    if (!category) return json(res, 400, { error: 'La categoría de gasto no pertenece al usuario.' })
