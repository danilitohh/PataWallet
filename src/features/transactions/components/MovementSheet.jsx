...
export function MovementSheet({ transaction, initialFlow = 'expense', onClose }) {
  ...
  const [flow, setFlow] = useState(startingFlow)
  ...
  const [showTransfer, setShowTransfer] = useState(startingFlow === 'transfer')
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (flow !== 'income' && !sourceOptions.some((item) => item.id === account)) throw new Error('Elige de dónde salió el dinero.')
      ...
      if (date > today()) throw new Error('El movimiento debe corresponder a un pago que ya ocurrió.')
      ...
      const record = {
        ...
        from_account_id: flow === 'income' ? null : account,
      ...
      <motion.section ref={dialogRef} inert={Boolean(accountDialog || categoryDialog)} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="movement-title" className="sheet" initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} transition={{ duration: 0.28, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <form onSubmit={submit}>
          ...
          {flow !== 'income' && <>
            ...
            <button className="button button--quiet" type="button" disabled={saving} onClick={() => setAccountDialog('source')}><Plus aria-hidden="true" /> Agregar cuenta con dinero</button>
          </>}
          {flow !== 'expense' && <>
