...
function PaydayPayments({ payments, fixedExpenses, nextPayDate, actions, notify, hidden }) {
  ...
  return <section className="feature-panel dashboard-payments" aria-labelledby="dashboard-payments-title">
    ...
    {payments.length ? <div className="dashboard-payments__list">{payments.map((payment) => {
      const paid = payment.status === 'paid'
      return <label className={`dashboard-payment ${paid ? 'dashboard-payment--paid' : ''}`} key={payment.id}>
        <input type="checkbox" checked={paid} disabled={Boolean(savingId)} aria-label={`Marcar ${payment.name} del ${formatDashboardDate(payment.dueDate)} como pagado`} onChange={(event) => togglePayment(payment, event.target.checked)} />
