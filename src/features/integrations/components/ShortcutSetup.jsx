...
export function ShortcutSetup({ integration, isDemo, notify }) {
  ...
  return <section className="integration-card">
    ...
    <ol className="steps shortcut-steps">
      ...
      <li><span>3</span><div><strong>Crear la automatización personal <CircleDashed /></strong><p>En Atajos: Automatización → Wallet → elegir tarjetas → crear un diccionario con <code>amount</code>, <code>merchant_name</code> y <code>card_alias</code> → ejecutar “{integration.template.shortcutName}” pasando ese diccionario. La app no puede confirmar este paso sola.</p></div></li>
