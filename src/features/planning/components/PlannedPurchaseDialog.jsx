...
export function PlannedPurchaseDialog({ purchase, close }) {
  ...
  return <SimpleDialog title={purchase ? 'Editar próxima compra' : 'Agregar próxima compra'} close={close}>
    <form onSubmit={async (event) => { event.preventDefault(); try {
      const clean = name.trim(); if (clean.length < 2) throw new Error('Escribe qué quieres comprar.')
      await actions.savePlannedPurchase({ id: purchase?.id || makeId('planned'), name: clean, amount_minor: parseLocalizedAmount(amount), currency: 'COP', target_date: targetDate, category_id: categoryId || null, note: note.trim(), status: purchase?.status || 'planned', version: purchase?.version || 1, updated_at: new Date().toISOString() })
      ...
      <button className="button button--primary" type="submit">Guardar próxima compra</button>
    </form>
  </SimpleDialog>
}
