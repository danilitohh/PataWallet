...
values ('account-a', '11111111-1111-4111-8111-111111111111', 'Cuenta A', 'asset', 'bank'),
       ('account-b', '22222222-2222-4222-8222-222222222222', 'Cuenta B', 'asset', 'bank');
...
insert into public.planned_purchases(id,user_id,name,amount_minor,target_date,category_id) values
  ('planned-a','11111111-1111-4111-8111-111111111111','Compra A',1000,current_date + 10,'category-a'),
  ('planned-b','22222222-2222-4222-8222-222222222222','Compra B',2000,current_date + 10,'category-b');
...
select throws_ok($$
  select public.mutate_transaction(
    ...
    jsonb_build_object(
      ...
      'currency', 'COP',
      'occurred_at', now(),
      'from_account_id', 'account-b',
      'category_id', 'category-a',
      'source', 'manual'
    ),
    null
  )
$$, '23503', null, 'A no puede referenciar la cuenta de B mediante la RPC');
...
select results_eq($$ select id from public.planned_purchases $$, array['planned-a'], 'A solo ve sus compras previstas');
select throws_ok($$ insert into public.planned_purchases(id,user_id,name,amount_minor,target_date,category_id) values ('cross-planned','11111111-1111-4111-8111-111111111111','Cruce',100,current_date,'category-b') $$, '23503', null, 'A no puede referenciar categorías de B en compras previstas');
...
select throws_ok($$ delete from public.accounts where id = 'account-a' returning id $$, '42501', null, 'La eliminación directa de cuentas está revocada');
...
select results_eq($$ select id from public.planned_purchases $$, array['planned-b'], 'B solo ve sus compras previstas');

reset role;
set local role anon;
select throws_ok($$ select id from public.accounts $$, '42501', null, 'Una solicitud anónima no puede leer cuentas');
