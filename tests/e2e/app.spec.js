...
test('completa el punto de partida y calcula el dinero libre', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Dinero libre estimado este mes' })).toBeVisible()
...
test('cambia y recuerda las tres vistas de Inicio', async ({ page }) => {
  ...
  await expect(page.getByText('Dinero libre estimado este mes').first()).toBeVisible()
...
test('distingue sueldo estimado de saldo real y permite crear la cuenta al pagar una deuda', async ({ page }) => {
  test.setTimeout(60000)
  await page.goto('/')
  await page.getByRole('button', { name: /probar con datos de ejemplo/i }).click()
  await page.goto('/cuentas')
  await page.getByRole('button', { name: 'Ingresos', exact: true }).click()
  await page.getByLabel('Nombre del ingreso').fill('Sueldo')
  await page.getByLabel('Sueldo mensual equivalente').fill('1.859.800')
  ...
  await page.goto('/')
  await expect(page.getByText('Dinero libre estimado este mes').first()).toBeVisible()
  await expect(page.getByText('Para pagar una deuda, registra primero la cuenta donde está tu dinero real.')).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true)
  const projectedBefore = Number((await page.locator('.dashboard-available-hero strong').first().textContent()).replace(/\D/g, ''))
  await page.getByRole('button', { name: 'Nuevo movimiento' }).click()
  const movement = page.getByRole('dialog', { name: 'Nuevo movimiento' })
  await movement.getByRole('button', { name: 'Pagué una deuda' }).click()
  await expect(movement.getByText('El dinero libre de Inicio es una estimación, no una cuenta.')).toBeVisible()
  await movement.getByRole('button', { name: 'Agregar cuenta con dinero' }).click()
  const account = page.getByRole('dialog', { name: 'Agregar cuenta' })
  await account.getByLabel('Nombre').fill('Mi banco')
  await account.getByLabel('Saldo inicial').fill('100.000')
  ...
  await page.goto('/')
  const projectedAfter = Number((await page.locator('.dashboard-available-hero strong').first().textContent()).replace(/\D/g, ''))
  expect(projectedAfter).toBe(projectedBefore - 10000)
  await page.goto('/cuentas')
  await expect(page.getByRole('heading', { name: 'Mi banco' })).toBeVisible()
  await expect(page.locator('.accounts-ledger__pane--asset .account-row').filter({ hasText: 'Mi banco' })).toContainText('90.000')
})
