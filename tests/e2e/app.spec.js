...
test('confirma un pago en Cuentas y conserva la marca al guardar y recargar', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('Arriendo de prueba marcado como pagado')).toBeVisible()
...
test('limita pagos quincenales a dos y confirma la marca también en Quincena', async ({ page }) => {
  ...
  await expect(confirmation).toBeVisible()
  await confirmation.getByRole('button', { name: 'Confirmar pago' }).click()
  await expect(page.getByText('Mercado quincenal marcado como pagado')).toBeVisible()
